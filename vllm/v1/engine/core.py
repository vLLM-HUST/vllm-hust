...
class EngineCore:
    ...
    def step_with_batch_queue(
        ...
        if self.scheduler.has_requests():
            ...
            # This pinned Request version has no in-flight token field. The
            # engine queue is the authoritative list of unretired outputs.
            in_flight_tokens: dict[str, int] = defaultdict(int)
            for _, queued_output, _, _ in batch_queue:
                for request_id, count in queued_output.num_scheduled_tokens.items():
                    in_flight_tokens[request_id] += count
