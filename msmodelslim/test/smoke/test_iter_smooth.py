...
@pytest.mark.smoke
def test_iter_smooth_sym_process(test_device, test_dtype):
    torch.set_default_dtype(test_dtype)
    tmp_dir = tempfile.mkdtemp()

    try:
        model_adapter = invoke_test("iter_smooth_sym.yaml", tmp_dir)
...
@pytest.mark.smoke
def test_iter_smooth_asym_process(test_device, test_dtype):
    torch.set_default_dtype(test_dtype)
    tmp_dir = tempfile.mkdtemp()

    try:
        model_adapter = invoke_test("iter_smooth_asym.yaml", tmp_dir)
        assert isinstance(model_adapter, FakeLlamaModelAdapter), "model_adapter should be FakeLlamaModelAdapter"
