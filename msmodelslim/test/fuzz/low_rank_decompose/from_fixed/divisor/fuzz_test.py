...
"""
Usage:
TEST_PATH=$PWD/test/fuzz/low_rank_decompose/from_fixed/divisor/
...
def fuzz_test(input_bytes):
    ...
    divisor = fdp.ConsumeInt(sys.maxsize)
    logging.info("divisor: %s", divisor)

    model = LrdSampleNetwork()
    decomposer = Decompose(model, "low_rank_decompose_test.json")

    try:
        decomposer.from_fixed(16, divisor=divisor)
