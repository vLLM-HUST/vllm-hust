...
"""
Usage:
TEST_PATH=$PWD/test/fuzz/low_rank_decompose/from_ratio/channel_ratio/
...
def fuzz_test(input_bytes):
    ...
    channel_ratio = fdp.ConsumeFloat()
    logging.info("channel_ratio: %s", channel_ratio)

    model = LrdSampleNetwork()
    decomposer = Decompose(model, "low_rank_decompose_test.json")

    try:
        decomposer.from_ratio(channel_ratio)
