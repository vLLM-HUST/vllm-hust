...
# 获取校准数据函数定义
def get_calib_dataset(fp16_tokenizer, calib_list):
    calib_dataset = []
    for calib_data in calib_list:
        text = calib_data['inputs_pretokenized']
        inputs = fp16_tokenizer([text], return_tensors='pt').to('cpu')
        print(inputs)
...
quant_config = QuantConfig(a_bit=16, w_bit=8, disable_names=disable_names, dev_type='cpu', act_method=3, pr=1.0, w_sym=False, mm_tensor=False)
calibrator = Calibrator(model, quant_config, calib_data=dataset_calib, disable_level='L0')
calibrator.run()  # 执行PTQ量化校准

calibrator.save(f"{os.environ['PROJECT_PATH']}/output/llm_ptq_w8a16_nsym", save_type=["numpy", "safe_tensor"])