...
# 清理output路径
rm -rf $PROJECT_PATH/output/multi_modal_quant_timestep_flux/*
...
python inference_flux.py \
    --path ${MODEL_PATH} \
    --save_path "$PROJECT_PATH/output/multi_modal_quant_timestep_flux/results/quant/img" \
    --device_id 0 \
    --device "npu" \
    ...
    --max_num_prompt 2 \
    --do_quant \
    --quant_weight_save_folder "$PROJECT_PATH/output/multi_modal_quant_timestep_flux/results/quant/safetensors" \
    --quant_dump_calib_folder "$PROJECT_PATH/resource/multi_modal_quant_timestep_flux/results/quant/cache" \
