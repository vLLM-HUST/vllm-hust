...
class TestWeightActivationQuantConfig:
    def test_weight_activation_quant_config_should_equal_to_old_quant_config_when_default_value(self):
        base_config = BaseConfig(w_bit=8, a_bit=8)
        weight_activation_quant_config = WeightActivationQuantConfig(base_config)
        old_quant_config = OldQuantConfig(w_bit=8, a_bit=8)
        assert compare_config_parameters(weight_activation_quant_config, old_quant_config) is True
